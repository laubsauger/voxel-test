...
describe('scene stamper (T20/T50/T51, V2, V5)', () => {
  ...
  it('is deterministic: same seed → identical chunk hash; different seed differs', () => {
    // Two INDEPENDENT fresh stamps of the same seed (not the shared `store`
    // fixture — other tests' getVoxel reads can materialise empty chunks in it,
    // polluting a whole-store hash). Production determinism verified byte-exact.
    const a = stamped(42)
    const a2 = stamped(42)
    expect(hashStore(a.store)).toBe(hashStore(a2.store))
    const b = stamped(43)
    expect(hashStore(b.store)).not.toBe(hashStore(a.store))
  }, 120000) // 3 full 5× world stamps — generous timeout so parallel-suite load can't trip it
