...
T84|.|[SPIKE] Box3DPhysicsWorld — full backend swap (user ask, V16). Implement a Box3D-backed drop-in matching Jolt PhysicsWorld surface (src/sim/physics.ts): initStatic(ChunkStore)→per-greedy-box static bodies, tick(sim) step+body sync, extractIsland/spawnDebrisBody→Box3D dynamic bodies (island grid → greedy boxes → 1 body per box, NO compound per V16), setBodyVelocity, impulseBodyAt, applyRadialImpulse, castRayBody, damageBodiesSphere, structuralPass, drainRemesh, dispose. Reuse connectivity.ts + destruction.ts + greedy-boxes UNCHANGED. Generalize DynamicBody.body typing minimally (shared file, surgical). Defer HARD Jolt features (char controller, VehicleConstraint, hinges) — spike targets destruction, not driving/walking|T78|I.box3d,V16,V14
