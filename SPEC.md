...
- Voxel 10cm. Arena fixed ~205×205×77m (2048×2048×768 voxel space, 64×64×24 chunks). No streaming. (Expanded from 100×100×50m per user 2026-07-02 — B11.) (Expanded again since — CURRENT: 160×160×24 chunks = 512×512×77m. PLANNED 2026-07-09: 256×256×24 = 819×819m for Bombay zone — T97, V21.)
...
- V18: bombay berm-reveal. Salton water INVISIBLE from every town-grid street (eye-height LOS probe, fixed seeds); from berm crest water VISIBLE + surface BELOW town street grade (look down). Sea never floods over berm.
- V19: bombay palette 80/15/5. Zone surface-voxel census: ≥80% bleached-neutral band, ~15% rust/decay browns, ≤5% saturated art chroma. Whole zone colorful = wrong.
- V20: bombay lot-condition mix 35/40/25 ±5% (lived-in / vacant husk / ruin[burned+collapsed]) INTERLEAVED — no street face runs >3 adjacent same-condition lots. Histogram + adjacency test per seed.
- V21: WORLD_CX/CZ=256 boot health: no tab OOM, steady heap ≤ 1.9GB (REVISED B39 — measured floor 1775MB with FULL compression: dense=0, all 75.6k mixed chunks palette; original ~1.5GB was a pre-impl research estimate that ignored palette overhead + 1.57M chunk headers + JS-side mesh buffers), compactStep sweep in boot (ignoreDirty — pre-drain dirty set covers the whole stamp) + kind-probes must never inflate (probeChunk/chunkAtRaw/denseView for whole-world scanners). P22 odd-road-count parity holds post-resize. Physics/render/LOD stay radius-bounded.
...
T56|x|[P] structural support heuristic: after edits, weak necks (small connection cross-section vs supported mass, per-material strength) break → collapse. Walls cave, undermined buildings crumble progressively. Region-scoped, deterministic, budgeted per tick|T55|B15,V1,V2
...
T60|x|[PL] player swimming: in-water detection, capsule buoyancy + drag, swim locomotion (deterministic), underwater camera tint/fog, splash events|T40|V1,V2
T61|x|[W] water visuals v2: animated surface normals/ripples (threejs-water-optics), fresnel + depth tint tuning, disturbance ripples, side-face rendering fix (B20), pool must read as WATER|T16|B20,§C
T62|x|[W] water CA v2: fix live mass loss/duplication (B21 — pool must drain when breached), lateral flow for falling water ('rolling' behavior, not pure up/down columns), faster settle|T15|B21,V9
...
T68|x|[C] hood district: rundown vibe — worn/boarded houses, chain-link fences, dumpsters, cracked asphalt+patches, faded paint, overgrown lots, corner store|T50|B6
...
T77|x|[PL] ragdoll physics: Jolt Ragdoll from the 6 voxel segments — triggers on death, hard falls, vehicle hits, explosion impulse over threshold; blends back to animated on recovery (or stays on death), deterministic + hashed|T64|V1,V2,V8
T73|x|[W] weapons: rocket launcher (fast projectile, impact-detonated T55 explosion, backblast+trail FX, hotbar slot) + TNT (placeable block prop, interact-to-light fuse ~3s spark, BIG zoned boom, chainable), SFX via pipeline|T54,T55|V1,V2
T74|x|[R] birds: occasional small flocks — simple boid-ish sprite/voxel birds, lazy loops over the town, day-only, render-only|T58|B22
T75|x|[R] flashlight: toggleable spotlight (L), camera-anchored in FP, warm beam, night-useful, subtle battery-free v1|T58|§C
...
T97|x|[R/P] destruction choppiness: SP catch-up cap 10→3 (compounding replay spiral was the 75-83ms frames — beyond cap sim time slips, frame never freezes; MP lockstep untouched) + RubbleBatch freeze-bakes budgeted 6/frame (blast pieces freeze together; 30-80 buildBodyGeometry in one frame was ~27ms)|T94|V11
T98|x|[R/P] mini test arena dev boot: ?world=mini (dev-server DEFAULT; prod default full, ?world= overrides) — hand-authored deterministic scene at spawn (3 houses, slim tower, gate, pool w/ water, road, 2 cars, plane), fully meshed in ~1.6s vs 15-30s+ full city. Map detached in mini. Harness: smoke/probes PIN world=full (publish-realistic gates); mp-e2e inherits mini (starvation fix). MP peers must boot same kind|T31|V2,I.boot
T99|x|[R] rideable transform interpolation: vehicles+aircraft snapshot prev transforms at tick start (never hashed); render lerps prev→current by driver alpha (nlerp quats) for chase/FP camera target + vehicle/aircraft meshes — kills the beat-frequency shudder past ~15 m/s (worst on planes). Wrecks/debris w/o prev fall back to raw|T90|V6,V11
T97|x|[CORE] WP0 bombay: WORLD_CX/CZ 160→256 (chunks.ts) + compactStep wired into boot stamp loop (boot peak mem = THE risk); verify spawn/roads/minimap auto-derive; fallback if slips: 0.3× diorama into park rim bi6-7/bj2-3||V21
T98|x|[C] WP-L bombay layout contract: DistrictKind 'bombay'+'bombayBeach', districtKindAt rules (town bi9-10/bj2-5, shore bi11+margin east), makeBombay() → streets/alleys/lots+condition enum/landmarks/berm polyline/playa extent/art list. Deterministic per seed. Per docs/research/bombay-beach.md §2|T97|V2,V20
T99|x|[C] WP1 bombay materials: MAT_SALT_CRUST, MAT_PLAYA_MUD, MAT_RUST, MAT_CHAR, MAT_BONE_SHELL, MAT_CRACKED_ASPHALT, MAT_GALV_METAL, opera-blue + 3-4 art pops. IDs reserved in INTEGRATION-content.md FIRST (B1 lesson)||V13,V19
T100|x|[C/W] WP2 bombay terrain: 2-3m seaward fall, trapezoid berm (40vox h, 50 crest, 140 base, noise-jitter crest, 3 ramp cuts: AveC/ESt/Aisle-end), playa salt crust + cracked mud + bone-shell band, sea waterFill east, surface 10-20vox below town. East-facing stampBeach generalization (axis param)|T98,T99|V18,V9
T101|x|[C] WP3 bombay streets: 6 avenues @300vox pitch (keep 'no Avenue E' quirk: A,B,C,E-St,F,Aisle-of-Palms), 5 streets @500/500/600/600vox gaps, 2 dirt alleys, dead-end stubs past 5th (B/C/E-St), Ave-A desert spur 600-800vox, 5th St bend, cracked-asphalt stamp (patches → mud/sand)|T98,T99|V2
T102|x|[C] WP4 bombay trailers: stampTrailer condition param — lived-in(35% carport/porch/fence/cooler/tank/car), vacant(40% dropped windows, graffiti patches, sand drift), burned(~10% MAT_CHAR shell, roof collapse), collapsed(~15% rubble box); + 84×32vox long single-wide + 2-3 CMU bungalows; wrecked-car + boat-hulk prop grids|T98,T99|V19,V20
T103|x|[C] WP5 bombay Ski Inn + civic: Ski Inn 100×80vox @ Ave A (tan+brown, flat roof, cooler, SKI-INN lettering, pole sign, walkable interior: bar counter/stools/jukebox/pool table/dollar-bill wall mat), Bombay Market, fire station, American Legion, church, comms mast + 3 dishes|T98,T99|V19
T104|x|[C] WP6 bombay art: Drive-In (2×4 wheel-less rust cars → white trailer screen @ E-St/berm), Opera House (#3E7FBF flip-flop speckle facade), TV wall (20-40 multicolor boxes @ 4th St), Da Vinci Fish 12m, text sign, star+barbed wire, rusted-scrap creature (NOT Breceda dinosaur — that's Borrego Springs)|T98,T99|V19
...
T106|x|[C] WP8 bombay approach: welcome sign @ spur junction, leaning utility poles, faded billboard, creosote scrub props, cosmetic SINGLE-track rail + poles on frame-road far side (verified: not double)|T98,T99|
...
B39|2026-07-09|T97 resize: (1) boot compactStep no-op — dirty set covers whole stamp, default skip excluded everything; (2) whole-world kind scanners (scanLampChunks, enqueueAll, loadRegion, isBuried) called chunkAt which INFLATES palette chunks on read — 42k chunks re-inflated (~1.3GB) after the sweep; (3) V21 1.5GB was estimate, measured floor 1775MB|V21 revised; ignoreDirty sweep; probeChunk/chunkAtRaw/denseView in scanners
