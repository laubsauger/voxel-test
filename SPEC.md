...
B30|2026-07-06|Box3D port-map: full Jolt PhysicsWorld swap has 3 hard blockers on box3d-wasm 0.2.0 — (1) non-convex island bodies inexpressible (no offset compound, hull convex-only) → debris = convex-hull approx; (2) CharacterVirtual player controller + VehicleConstraint/motorcycle UNPORTABLE (no char/vehicle rig); (3) f32 + diff body decomposition → no hashPhysics parity w/ Jolt, box3d = own non-MP build. Spike scoped to DESTRUCTION pipeline (portable); players/vehicles = documented gap, keep Jolt or custom rebuild|T84 IPhysicsWorld extraction + Box3DPhysicsWorld destruction subset; V16 records compound constraint
