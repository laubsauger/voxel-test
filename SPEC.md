...
- V21: WORLD_CX/CZ=256 boot health: no tab OOM, steady heap ≤ ~1.5GB, compactStep (chunks.ts) wired into boot stamp loop (not background-only). P22 odd-road-count parity holds post-resize. Physics/render/LOD stay radius-bounded (frame cost ≈ const vs world size).
...
T97|.|[CORE] WP0 bombay: WORLD_CX/CZ 160→256 (chunks.ts) + compactStep wired into boot stamp loop (boot peak mem = THE risk); verify spawn/roads/minimap auto-derive; fallback if slips: 0.3× diorama into park rim bi6-7/bj2-3||V21
